...
import { environment } from "@/services/auth";

export const getEncrypt = (key: string): string => {
  return CryptoJS.AES.encrypt(key, environment.encryptKey).toString();
};

export const getDecrypt = (key: string): string | any => {
  try {
    if (typeof key === "string") {
      const decrypted = CryptoJS.AES.decrypt(key, environment.encryptKey);
      if (decrypted) {
...
export const getLocalStorage = (key: string): any | null => {
  ...
  try {
    const decryptedData = CryptoJS.AES.decrypt(keyData.trim(), environment.encryptKey);
