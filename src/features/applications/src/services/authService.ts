...
const HTTP_ENCRYPT_KEY =
  import.meta.env.VITE_HTTP_ENCRYPT_KEY || window.env?.["ENCRYPTION_KEY"];
const API_ENDPOINT =
  import.meta.env.VITE_API_ENDPOINT || window.env?.["API_ENDPOINT"];
...
export function setLocalStorageItem(key: any, value: any) {
  if (value) {
    ...
    localStorage.setItem(
      key,
      CryptoJS.AES.encrypt(valueToStore, HTTP_ENCRYPT_KEY).toString()
    );
  } else {
...
export const SERVICE_BASE_URLS: Record<string, string> = {
  ...
  oauthService: `${API_ENDPOINT}/oauth-service/oauth/v1/`,
...
function encryptData(data: unknown): string {
  ...
  return CryptoJS.AES.encrypt(payload, SERVICE_BASE_URLS.encryptKey).toString();
}

function decryptData(cipherText: string): any {
  const decrypted = CryptoJS.AES.decrypt(cipherText, SERVICE_BASE_URLS.encryptKey);
