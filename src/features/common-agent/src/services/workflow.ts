...
const SERVICE_BASE_URLS: Record<string, string> = {
  ...
  slackService: `${API_ENDPOINT}/slack-service`,
  ...
  gatewayService:  `${API_ENDPOINT}/gateway-service/application/v1`,
   ...
   if (key) {
      ...
      if (keyData) {
        ...
        if (decrypteDATA) {
          ...
          console.log("Decrypted data string for key:", key, JSON.parse(decryptData));
          try {
            try {
...
export function setLocalStorageItem(key: any, value: any) {
 ...
 console.log("Data stored in localStorage for key:", key);
    } else {
