...
export const CustomFieldsConfig = () => {
    ...
    const { loading, createField, updateField, loadFields, customFields, deleteField } = useOpportunityConfigStore();
    ...
    const allFields = [
        { name: "Title", type: "text", required: true, description: "", example: "", key: "Title", placeholder: "Title", isDefault: true },
        { name: "Reason", type: "textarea", required: true, description: "", example: "", key: "Reason", placeholder: "Reason", isDefault: true },
        { name: "Value", type: "text", required: true, description: "", example: "", key: "money", placeholder: "Amount", isDefault: true },
        { name: "Stage", type: "dropdown", required: true, description: "", example: "", key: "Stage", placeholder: "Select Stage", isDefault: true },
        { name: "Expected Close Date", type: "date", required: false, description: "", example: "", key: "Expected Close Date", placeholder: "mm/dd/yyyy", isDefault: true },
        { name: "Source", type: "text", required: false, description: "", example: "", key: "Source", placeholder: "Source", isDefault: true },
        ...customFields
    ]
    ...
    const handleSubmit = async (values: FormValues) => {
        ...
        if (editField) {
            ...
            success = await updateField(payload)
        } else {
            const payload = {
                ...values,
                options
            }
            success = await createField(payload)
        }
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-600 mt-1">
                        Manage the fields that appear on Opportunity forms and records.
                    </p>
                </div>
                    ...
                    >
                        ...
                        <div>
                            <Switch
                                ...
                                disabled={field.isDefault || loading.editingField}
                            />
                        </div>
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            {!field.isDefault &&
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-red-500 hover:text-red-700" onClick={() => deleteField(field.name)}>
                                    <Trash2 size={14} />
                                </Button>
                            }
                        </div>
            ...
            <Dialog open={dialogOpen} onOpenChange={handleCloseDialog}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {editField ? 'Update the field configuration.' : `Add a new field to Opportunity records.`}
                        </DialogDescription>
                    </DialogHeader>
                    <Form {...form}>
                        <form className="space-y-4" onSubmit={form.handleSubmit(handleSubmit)}>
                            ...
                            <DialogFooter>
                                ...
                                <Button disabled={loading.creatingField || loading.editingField}>{editField ? "Update Field" : "Add Field"}</Button>
                            </DialogFooter>
                        </form>
                    </Form>
                </DialogContent>
